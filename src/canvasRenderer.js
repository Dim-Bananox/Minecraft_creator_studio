export function createCanvasRenderer(deps) {
  ...
  const updateCanvasInteraction = () => {
    const toolActive = state.textMode || state.shapeMode || state.drawMode === "pen" || state.drawMode === "eraser";
  ...
  function updateBrushCursor() {
    ...
    if (state.drawMode === "pen" || state.drawMode === "eraser" || (state.shapeMode && state.shapeType === "line")) {
  ...
  function handleSelectionMove(e) {
    ...
    if (!obj) return;
    const coords = getCanvasCoordinates(e);
    if (!state.canvasDragBoost && (state.transformData.mode === "move" || state.transformData.mode === "resize")) {
    ...
    if (state.activeDrawObject.type === "stroke" || state.activeDrawObject.type === "eraser") {
      ...
      if (pts.length > 1) {
        if (state.activeDrawObject.type === "eraser") {
          ...
          const centerX = (minX + maxX) / 2;
          const centerY = (minY + maxY) / 2;
          state.activeDrawObject.points = pts.map(p => ({ x: p.x - centerX, y: p.y - centerY }));
